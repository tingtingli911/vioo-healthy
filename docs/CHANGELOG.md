...
- 🐛 修复体重管理和血糖管理页面 recharts 错误
  - 修复 `Cannot read properties of null (reading 'useRef')` 错误
  - 使用 `defineAsyncComponent` 动态导入 recharts 组件，避免初始化时的兼容性问题
  - 添加组件挂载状态检查，确保 recharts 组件在挂载后才渲染
  - 使用条件渲染和延迟加载避免在组件未完全初始化时访问 recharts API
  - 使用 `nextTick`、延迟和 `requestAnimationFrame` 确保 DOM 完全渲染后再渲染图表
